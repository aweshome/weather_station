...
void USART_Init( uint16_t baud )
{
	...
	UBRR1L = (uint8_t)baud;
	/* Za³¹czenie nadajnika i odbiornika oraz w³¹czenie przerwañ */
...
{
	register char c;
	while((c = pgm_read_byte(s++)))//podwojne nawiasowanie jest konieczne
	{
       	uart_putc(c);
	}
}
...
ISR( USART1_RX_vect )
{
    ...
    data = UDR1; //pobieramy natychmiast bajt danych z bufora sprzêtowego
